...
describe('Context parser', () => {
  describe('When context is empty', () => {
    test('Should return an error?', () => {
      const cp = new ContextParser(existingConfigDir, {contextDir: 'emptycontext'});
  ...
  describe('When loading a sparse context', () => {
    test('Should return a parsed context', () => {
      const cp = new ContextParser(existingConfigDir, {contextDir: 'sparsecontext'});
      expect(cp.context).toStrictEqual({'layer1': {'layer2': {'test': 'test'}}});
    });
  });
  ...
  describe('When context is present', () => {
    ...
    test('Should return a selection of the context', () => {
      expect(cp.fromSelector('development')).toMatchObject(expectedContext.development);
      expect(cp.fromSelector('development.components')).toStrictEqual(expectedContext.development.components);
      expect(
          cp.fromSelector(['development', 'components']),
      ).toStrictEqual(expectedContext.development.components);
      expect(
          cp.fromSelector({'base': 'development.components.database', 'assign': [expectedContext]}),
      ).toStrictEqual(
          {
      ...
      expect(cp.fromSelector({base: 'development', assign: ['staging']})).toStrictEqual({
        environment: 'staging',
        components: {
          application: {
            replicas: 3,
      ...
      expect(cp.fromSelector({base: 'development', mergeWith: ['staging']})).toStrictEqual({
        environment: 'staging',
        components: {
          database: {
            size: 5,
