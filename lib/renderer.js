...
class Renderer {
   ...
   * @param {string} base basefolder where to find the templates
  ...
  _renderJs(templatePath, context) {
    ...
    if (jsTemplate.render == undefined) {
      throw new Error(`Destination ${templatePath} does not have a render function`);
    }
  ...
  render(template, context) {
    ...
    return result != undefined ? result : this.templateEngine.render(templatePath, context);
  }
}
